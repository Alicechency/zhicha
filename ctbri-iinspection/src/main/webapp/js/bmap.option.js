...
function setBmapOption() {
    ...
    // 坐标初始化(初始坐标为北京市公安局朝阳分局)
    bmap.centerAndZoom(new BMap.Point(116.49338, 39.930028), 14);

    // 限制百度地图的缩放级别
    bmap.setMinZoom(12);

    // 限制百度地图的拖拽区域(手动查找进行限制)
    ...
    for (var i = 0; i < localSearchData.length; i++) {
        var marker = new BMap.Marker(new BMap.Point(localSearchData[i][0], localSearchData[i][1]));
        ...
        // 给标记添加点击事件，暂时注释，不加载侧边栏
       /* marker.addEventListener('click', function() {
            $('#side-chart .panel-title').text('信息');
            $('#side-chart-container').css('display', 'none');
            $('#side-station-info').css('display', 'block');
